...
client2.user.setGame(`^help | Free Rewards.`) //    ,"http://twitch.tv/S-F"
client2.channels.get("539927972270833695").join(); 
  console.log('')
  console.log('')
  console.log('╔[═════════════════════════════════════════════════════════════════]╗')
  console.log(`[Start] ${new Date()}`);
...
client2.on('message', message => {//for dev
  var argresult = message.content.split(` `).slice(1).join(' ');
    if (!devs.includes(message.author.id)) return;
...
if (message.content.startsWith(adminprefix + 'setT')) {
  client2.user.setGame(argresult, "https://www.twitch.tv/faresgameryt");
    message.channel.sendMessage(`**تم تغيير تويتش البوت إلى  ${argresult}**`)
}
...
client2.on('message', message => { 
    ...
    if (!message.channel.guild) return;
    if(!message.guild.member(message.author).hasPermission("MANAGE_ROLES")) return message.reply("**:no_entry_sign:انت لا تملك صلاحيات **").then(msg => msg.delete(5000));;
    if(!message.guild.member(client2.user).hasPermission("MANAGE_ROLES")) return message.reply("البوت لايملك صلاحيات ").then(msg => msg.delete(5000));;
    let user = message.mentions.users.first();
    if (message.mentions.users.size < 1) return message.reply('**ضع منشن الشخص!!**').then(msg => {msg.delete(5000)});
    let MRole = message.content.split(" ").slice(2).join(" ");
    if(!MRole)return message.reply("يجب عليك وضع اسم الرتبة").then(msg => {msg.delete(5000)});
...
client2.on("message", message => {
 if (message.content === "^invite") {
  const embed = new Discord.RichEmbed()
      ...
      .setDescription(`
 **
https://discordapp.com/api/oauth2/authorize?client2_id=539862959870967808&permissions=0&scope=bot
**
`)
...
//client2.on("ready", () => { // كود رينبو
  //function lol() {
    //client2.guilds.get('539870654816583680').roles.find("name", "*").setColor("RANDOM");
  //};
  //setInterval(lol, 350);
//});
...
client2.on('message', message => {
      ...
      if(message.mentions.users.size > 1) return message.reply(' 😳 **ولد ما يصحلك الا حرمة وحدة كل مرة**').catch(console.error);
       if(proposed === message.author) return message.reply(`**خنثى ؟ **`);
        if(proposed === client2.user) return message.reply(`** تبي تتزوجني؟ **`);
              message.channel.send(`**${proposed} 
 بدك تقبلي عرض الزواج من ${message.author} 
 العرض لمدة 15 ثانية  
 اكتبي موافقة او لا**`)
